...
_EXTERNAL_MODEL_DIR = Path(__file__).parent.parent / "external" / "all-MiniLM-L12-v2"


class _OnnxLoader:
	"""Lazy-loads the ONNX session and tokenizer exactly once."""
	...
	def try_load(self) -> bool:
		"""Returns True if the ONNX model is available and loaded."""
		if self._tried:
		...
		try:
			...
			self._tokenizer = tok
			print("[raptor] Using local ONNX embedding model (all-MiniLM-L12-v2).", file=sys.stderr)
			return True
...
def _embed_with_onnx(texts: list[str], *, prefix: str, dim: int) -> list[list[float]]:
	...
	encodings = _ONNX_LOADER.tokenizer.encode_batch(prefixed)
	input_ids = np.array([enc.ids for enc in encodings], dtype=np.int64)
	attention_mask = np.array([enc.attention_mask for enc in encodings], dtype=np.int64)
	input_names = {inp.name for inp in _ONNX_LOADER.session.get_inputs()}
	feed: dict[str, Any] = {"input_ids": input_ids, "attention_mask": attention_mask}
	if "token_type_ids" in input_names:
		feed["token_type_ids"] = np.zeros_like(input_ids)

	outputs = _ONNX_LOADER.session.run(None, feed)
