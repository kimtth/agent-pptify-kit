...
Standalone plugin scripts for source ingestion, design context, image assets, extraction, and audit helpers used by the pptify deck-building agent and skills. CLI-style tools can be called directly by path and write a JSON payload to stdout. Extraction helpers are import APIs.
...
- `documents/document_to_raptor_tree.py` uses restored local ONNX embeddings when `onnxruntime`, `tokenizers`, and the external MiniLM assets are available; otherwise it falls back to stable local embeddings that require only the standard library.
- `design/design_context_catalog.py` reads local `../resources/design` metadata and requires only the standard library.
...
The script downloads MiniLM tokenizer files plus the selected ONNX model into `pptify/skills/pptify-tooling/scripts/external/all-MiniLM-L12-v2`. The default MiniLM model is `onnx/model_quint8_avx2.onnx`, saved locally as `model_quantized.onnx`; pass `-MiniLmModelPath onnx/model.onnx` for the larger non-quantized model.

## Image Provider Access

`images/text_prompt_to_infographic.py` loads image-provider settings from `.env` before it runs. When image generation needs credentials or provider configuration, copy `../resources/env.template` to `.env`, ask the user to fill the required values directly in that file, then run the helper. Do not ask the user to paste API keys, tokens, or connection strings into chat or a prompt input dialog.
...
# Edit .env, then run:
uv run python pptify/skills/pptify-tooling/scripts/images/text_prompt_to_infographic.py --provider azure-openai --size "1024x1024" --prompt "Cloud governance roadmap" --output-path infographic.png --pretty
```
...
```powershell
uv run python pptify/skills/pptify-tooling/scripts/images/text_prompt_to_infographic.py --provider openai --size "1024x1024" --prompt "Cloud governance roadmap" --output-path infographic.png --pretty
```
...
- `design/design_context_catalog.py` - list or emit source-backed predefined design template and agent-prompt context from `../resources/design`.
...
```powershell
uv run python pptify/skills/pptify-tooling/scripts/documents/document_to_markdown.py --source source.pdf --output-path source.md
uv run python pptify/skills/pptify-tooling/scripts/documents/document_to_raptor_tree.py --markdown-path source.md --output-path source.structured-summary.json --title "Source"
uv run python pptify/skills/pptify-tooling/scripts/design/design_context_catalog.py --profile primer-primitives --include-context --pretty
uv run python pptify/skills/pptify-tooling/scripts/images/web_image_search.py --query "Power Platform governance" --max-num 8
uv run python pptify/skills/pptify-tooling/scripts/images/iconfy_search.py --query governance --collection fluent --color 0078D4
uv run python pptify/skills/pptify-tooling/scripts/images/raster_image_to_svg.py --source logo.png --mode vector-trace --output-path logo.svg
uv run python pptify/skills/pptify-tooling/scripts/images/text_prompt_to_infographic.py --provider openai --prompt "Cloud governance roadmap" --output-path infographic.png
```