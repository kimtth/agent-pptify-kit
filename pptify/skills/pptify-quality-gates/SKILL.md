...
- Save the selected `skills/pptify-tooling/resources/design` profile IDs, source URLs, license IDs, and style lock details in `summary.design_context` for every newly generated deck unless a user-provided brand guide or reference PPTX is the primary style source.
...
- If the deck looks like default `python-pptx`, load a design profile from `skills/pptify-tooling/resources/design`, add `summary.design_context`, choose a style lock, and rebuild with explicit background/accent/card/rule primitives.
