...
- Confirm every generated slide uses `layout_tree` and does not rely on obsolete render-time shorthand keys such as `pattern`, `layout_pattern`, `layout`, `sections`, `bullets`, `objects`, or `theme` unless a restored renderer explicitly supports them.
- Confirm text-bearing objects have explicit `style.font_size` and `style.color`, line objects have explicit endpoints and line styles, and shape objects have explicit shape names plus fill/line styles.
