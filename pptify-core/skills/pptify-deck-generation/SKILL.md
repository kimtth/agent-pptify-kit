...
2. For URL-based, topic-plus-research, source-backed, or multi-source decks, build a combined markdown corpus and run `pptify-plugin/documents/document_to_raptor_tree.py` before planning slides. Do not reduce the source material to a few searched keywords.
3. Record corpus path, RAPTOR summary path, source count, and source URLs in `summary.source_enrichment`.
...
6. For every new deck, select a source-backed profile from `pptify-design/sources.json` and load it with `uv run python pptify-plugin/design/design_context_catalog.py --profile <id> --include-context --pretty`, unless a user-provided brand guide or reference PPTX is the primary style source. Default to `fluent-ui-design-tokens`; use `corazzon-pptx-design-styles` only when a broader modern style catalog is explicitly useful.
...
2. Every generated slide must include `layout_tree` with `slide_size`, `root_group_id`, explicit groups, explicit objects, final bboxes, explicit font sizes/colors, and explicit shape/line styles.
...
- Do not use `pattern`, `layout_pattern`, `composition.pattern`, `layout`, `sections`, `bullets`, `objects`, `theme`, or browser layout requests as render-time shorthand.
- Each object must include `kind`, `role`, `classification`, `content`, `style`, `bbox`, and `z_index`.
...
- Collision and overflow audit findings are repaired by editing the coordinates, sizes, text, or font sizes in the JSON spec.
