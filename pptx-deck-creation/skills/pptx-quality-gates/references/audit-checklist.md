...
- **Fail:** any overlap ΓåÆ move objects, resize bboxes, reduce content density, or split the slide.
...
- Characters per line Γëê `(bbox.w ├ù 10) / font_size`
- Lines available Γëê `(bbox.h ├ù 72) / (font_size ├ù 1.2)`
  _(bbox in inches, font_size in pt)_

Adjustments:
- **CJK / full-width text:** halve the characters-per-line value (full-width glyphs Γëê 2├ù Latin advance). The extractor reports `non_ascii_text` ΓÇö use it to flag CJK-heavy slides.
- **Text on a shape/card:** subtract Γëê0.1 in of inner padding from each side of the shape before computing capacity; the text occupies the inset inner area, not the full shape.

- **Pass:** estimated text volume Γëñ available capacity.
- **Fail:** likely overflow ΓåÆ shorten bullets, enlarge bbox, or split slide.
...
- **Pass:** all content objects ΓëÑ 9 pt.
- **Fail:** any content object < 9 pt ΓåÆ increase font size and split content if needed.

## 4. Design Context Presence

Inspect `summary.design_context` in the spec root.

- **Pass:** the field identifies either an approved design profile (with
  `profile_id`, source URL, and license ID) or a user-provided brand guide or
  reference PPTX as the primary style source.
- **Fail—any of the following:**
  - `summary.design_context` absent -> document the user-provided brand guide
    or reference deck, or load a design profile from [`references/design-profiles.md`](../../pptx-deck-context/references/design-profiles.md) in `pptx-deck-context`, then rebuild.
...
- **Pass:** at least one style-derived visual element present ΓÇö accent band, card shell, grid cell, rule/divider, shape motif, image treatment, or background pattern.
- **Fail:** slide is plain white with only text objects ΓåÆ add a design element derived from the selected profile's `source_signals`.
...
- For action-title frameworks (e.g., `action-title`, McKinsey): every content slide has an **action title** (not a descriptive label). Run the ghost-deck test: read only slide titles ΓÇö they must tell the full story on their own.
...
- **Fail:** image covers text ΓåÆ lower `z_index`, adjust bbox, or reclassify as `classification: "layout_design"`.
...
For every slide, check each object against the slide rectangle (0,0)ΓÇô(`slide_size.width`, `slide_size.height`):
...
- **Fail:** an object extends off-slide or into the margin ΓåÆ move or resize it inside. Only `classification: "layout_design"` full-bleed bands may touch or cross an edge.

## 10. Containment

- **Pass:** every child object and child group fits inside its parent group `bbox`; on-shape text fits inside the shape minus Γëê0.1 in inner padding.
- **Fail:** a child spills out of its group, or card text spills past the card padding ΓåÆ resize the child or the parent, or split content.

## 11. Table Fit

For every `kind: "table"` object:

- **Pass:** column widths sum to the table `bbox.width`, each cell's wrapped text fits its row height at the cell font size, and row count is within the per-slide budget (Γëê8ΓÇô10 body rows at 10ΓÇô11 pt).
- **Fail:** columns overflow the table width, cells clip, or the table is too tall ΓåÆ rebalance columns, raise row height, or split the table across slides (repeat the header).
...
- **Fail:** a single raster or SVG contains the only meaningful text, data,
  explanation, chart values, or diagram labels ΓåÆ recreate that information with
