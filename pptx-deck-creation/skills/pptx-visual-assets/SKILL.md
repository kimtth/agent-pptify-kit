...
search, rasterΓåÆSVG, textΓåÆinfographic, NotebookLM bridge). Run the snippet in an ephemeral scratch
...
If [references/visual-asset-adapters.md](references/visual-asset-adapters.md) cannot be read ΓÇö or the specific guideline section required for the current asset type is absent or unreadable ΓÇö halt that asset's acquisition, report what is missing to the user, and do not attempt to reconstruct guideline snippets from memory.
...
Infographic generation runs through a user-managed provider; the skill bundles no generation scripts. Follow the **Text ΓåÆ Infographic** and optional **NotebookLM** guidelines in [references/visual-asset-adapters.md](references/visual-asset-adapters.md).
...
4. **Call the provider** to generate the infographic.
   - On failure (call errors or returns no usable output): record the attempt manifest as failed (step 5) and omit the infographic object ΓÇö never place a local placeholder or stub. If that manifest write also fails, follow step 5's halt rule and report both the provider failure and the write failure in chat.
